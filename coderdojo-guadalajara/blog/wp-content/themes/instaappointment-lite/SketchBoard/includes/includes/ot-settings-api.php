        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
            ...
            echo '	<div class="donate-info">
					<strong>'. __( "To Activate All Features, Please Upgrade to Pro version!", "instaapp" ) .'</strong><br/>
					<a title="'. __( "Upgrade to Pro", "instaapp" ) .'" href="'. esc_url($sktlink) .'" target="_blank" class="upgrade">'. __( "Upgrade to Pro", "instaapp" ) .'</a>	<a title="'. __( 'Setup Instructions', 'instaapp' ) .'" href="'. get_template_directory_uri().'/Installation_Instructions.txt' .'" target="_blank" class="donate">'. __( 'Setup Instructions', 'instaapp' ) .'</a>
					<a title="'. __( 'Rate Insta Appointment Lite', 'instaapp' ) .'" href="'. esc_url($sktratelink) .'" target="_blank" class="review">'. __( 'Rate InstaAppointment Lite', 'instaapp' ) .'</a>
					<div id="social-share">
					<div class="fb-like">
